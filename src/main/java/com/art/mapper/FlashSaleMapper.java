...
public class FlashSaleMapper {
    ...
    public static FlashSaleDTO convertToDto(FlashSale flashSale, PromotionalDetailsDAO proDao, FlashSaleDAO fDAO) {
        ...
        flashSaleDTO.setPromotionalDetailsList(getPromotionalDetailsListDTO(flashSale, proDao, fDAO));
        return flashSaleDTO;
    }

    private static List<PromotionalDetailsListDTO> getPromotionalDetailsListDTO(FlashSale flashSale,
            PromotionalDetailsDAO proDao, FlashSaleDAO fDAO) {
        return flashSale
                .getPromotionalDetailsList().stream().map(pmt -> new PromotionalDetailsListDTO(
                        pmt.getId(), ProductMapper.convertToDto(pmt.getProduct(), proDao, fDAO),
