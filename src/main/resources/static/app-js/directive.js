...
app.directive("customFlashsale", function($timeout, ApiService, $rootScope) {
	return {
		restrict: "E",
		...
		link: function(scope, element) {
			...
			ApiService.callApi("GET", "/rest/flash-sale-active")
				.then(function(resp) {
					...
					$rootScope.endDaySale = scope.formattedEndDay;
				})
				.catch(function(err) {
					console.log(err);
				});
...
app.directive("slideProductOther", function($timeout) {
	scope.slickFlashsale = function() {
		element.find(".listCarousel-product--order").owlCarousel({
			items: 4,
			autoplay: true,
			autoplayTimeout: 5000,
			smartSpeed: 400,
